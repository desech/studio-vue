...
module.exports = {
  ...
  syncJsCode (data, lib) {
    ...
    this.syncJsRouter(data.folder, data.htmlFiles)
  ...
  syncJsRouter (folder, htmlFiles, module) {
    ...
    const content = ParseCode.getJsRouter(js, folder, htmlFiles)
    File.writeToFile(file, content)
  },
  ...
  syncJsModule (data, file, lib) {
    ...
    js = ParseCode.parseClassJsCode(js, file, data, lib)
    if (!js) return false
  ...
  getClassJSCode (file, clsName, isComponent) {
    ...
    const templateFile = isComponent ? 'ClassComponent.vue' : 'ClassPage.vue'
