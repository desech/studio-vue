...
module.exports = {
  getIndexHtml (html) {
    ...
    html = html.replace(/<body>[\s\S]*<\/body>/g, `<body>${os.EOL}  <div id="app"></div>` +
      `${os.EOL}</body>`)
    return html
  },
  ...
  parseClassJsCode (js, file, JSDOM) {
    ...
    js = this.getCodeTemplate(js, new JSDOM(body))
    return js
  },
  ...
  getCodeImport (htmlFile) {
    ...
    return `import ${component} from '@${file}'${os.EOL}`
  },
  ...
  getCodeTemplate (js, dom) {
    const html = ParseRender.getTemplateBlock(dom)
    return js.replace(/(<template>)([\s\S]*?)(<\/template>)/g, `$1${os.EOL}  ${html}${os.EOL}$3`)
  }
}
