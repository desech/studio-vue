...
module.exports = {
  ...
  parseClassJsCode (js, file, relBase, data, lib) {
    ...
    js = this.injectJs(js, this.getComponentInstances(components), 'instance', 4)
    js = this.injectJs(js, this.getComponentPropTypes(render.component), 'props', 4)
    return js
  },
  ...
  getComponentInstances (components) {
    if (!components.length) return
    const list = []
    for (const file of components) {
