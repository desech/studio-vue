...
module.exports = {
  ...
  parseClassJsCode (js, file, relBase, data, lib) {
    ...
    js = this.injectJs(js, this.getImports(components, relBase, overrideImports), 'import')
    js = this.injectJs(js, this.getInstances(components), 'instance', 4)
    js = this.injectJs(js, this.getPropTypes(render.component), 'props', 4)
    return js
  },
  ...
  getImports (components, relBase, overrideImports) {
    const list = []
    list.push(`import DS from '${File.relative(relBase, '/lib/DS.js')}'`)
    this.addClassComponentImports(list, components, relBase)
    this.addInstanceOverridesImport(list, relBase, data.folder, overrideImports)
    return ExtendJS.unique(list).join('\n') + '\n'
  },

  addClassComponentImports (list, components, relBase) {
    this.addComponentRecords(list, components, (instance, file) => {
      const clsPathRel = File.relative(relBase, '/' + file)
      const dottedPath = clsPathRel.startsWith('.') ? clsPathRel : './' + clsPathRel
      return `import ${instance} from '${dottedPath}'`
    })
  },
  ...
  addComponentRecords (list, components, callback) {
    for (const relFile of components) {
      ...
      list.push(callback(instance, file))
    }
  },

  getInstances (components) {
    const list = []
    this.addComponentRecords(list, components, instance => `    ${instance}`)
